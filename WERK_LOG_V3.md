...
**🔴 Notify-dispatcher GEBOUWD + WhatsApp-verzending GETEST (scenario `6002032`, ACTIEF):** `[PORTAL v2] notify-dispatcher`, map 348572, hook `ada8k6ss2o91qpjpkrvgkp6rbv7jxano`. Flow: webhook{contact_id,kind,project,onderwerp,datum} → ClickUp getATask → leest Notificatie-voorkeur+GSM+e-mail+voornaam → bepaalt kanaal + **ContentSid per `kind`** (de 5 approved templates) + bouwt **ContentVariables** JSON → **WebhookRespond** (routing-beslissing) → **HTTP POST naar Twilio Content API** (Basic Auth; Auth Token enkel in Make, NIET in git). **✅ WhatsApp END-TO-END AFGELEVERD (geverifieerd 02-06):** echte test → `to:+32470507449 from:+15559791498 status:DELIVERED` (Twilio API bevestigd). De volledige keten werkt: webhook → ClickUp → routing → ContentSid → ContentVariables → Twilio Content API → WhatsApp toegekomen. Sender `+15559791498` is een geldige Twilio WhatsApp-sender (niet de placeholder die ik vreesde). **Geleerd:** Make-IML heeft géén `chr()` (JSON via template-strings `{"1":"{{..}}"}`); native Twilio-module kan géén Content-templates (enkel `to`+`body`) → daarom HTTP Content API; `http`-module vereist `followAllRedirects` als `followRedirect:true`; execution-detail toont geen module-output → verzendstatus checken via Twilio's Messages-API. Testcontact Jan (`86ca0f6um`) GSM = `+32470507449` (Vincents testnummer, bewust zo gelaten). 🔒 Auth Token enkel in Make-module 7, niet in git/memory (secret-scan groen) — **roteren aangeraden** want in chat gedeeld. Chatbot-suggestiechips ook verrijkt.
- **🟠 Reminder-cron GEBOUWD (scenario `6005118` `[PORTAL v2] feedback-reminder (cron)`, INACTIEF — veilig):** elke 2 dagen → ClickUp-query `status=doorgestuurd` → per taak Bedrijf→Contact resolven → dispatcher `feedback_herinnering`. Filtert taken met Feedback-status Ontvangen(1)/Auto-goedgekeurd(2) eruit. **Bewust NIET geactiveerd** want hij zou élk doorgestuurd-project met een contact aanschrijven — Vincent: nakijken/scopen → dan activeren. (Structuur valide; contactveld `1bce8db8` uit memory — leeg veld ⇒ geen send, fail-safe.)
- **Nog te bouwen (echt laatste mijl):** e-mail-tak in de dispatcher voor "Beide"/"E-mail" (router + e-mailmodule, from-adres kiezen) + triggers voor `oplevering_klaar`/`plan_moment` (bv. ClickUp-automation "status→doorgestuurd → webhook naar dispatcher-hook").
