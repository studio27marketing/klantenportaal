...
**🟠 Notify-dispatcher GEBOUWD + getest (sandbox, scenario `6002032`, ACTIEF maar onschadelijk):** nieuw `[PORTAL v2] notify-dispatcher` in map 348572 (hook `ada8k6ss2o91qpjpkrvgkp6rbv7jxano`). Leest een contactpersoon (Notificatie-voorkeur + GSM + e-mail) en geeft de **routing-beslissing** terug: `{send_whatsapp, send_email, to_gsm, to_email, voorkeur}`. Curl-getest op Jan Janssens (geen voorkeur → default **WhatsApp**): `send_whatsapp:yes, send_email:no, to_gsm:+32470093007`. ⚠️ Het is een **pure responder** (geen writes, geen verzending) → veilig, doet niets tot iets z'n hook aanroept. De **verzend-modules** (WhatsApp Business Cloud `5770466` / e-mail) + de reminder-cron zijn de "laatste meter" — WhatsApp wacht nog op je **Meta-template**. Chatbot-suggestiechips ook verrijkt (meeting/feedback/nieuw project).
