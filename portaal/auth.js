...
const S27TeamAuth = {
  async init(opts) {
    ...
    armAuthWatchdog();
    try { await load(); }
    catch (e) { emit('signed_out', { error: friendly(e) }); return; }
    if (authListenerStarted) {
      if (auth && auth.currentUser) {
        const email = String(auth.currentUser.email || '').trim().toLowerCase();
        emit(/@studio27\.be$/.test(email) ? 'ready' : 'no_access', { user: auth.currentUser, email });
      } else if (lastState) {
        emit(lastState.phase, lastState);
      }
      return;
    }
  ...
  async google() {
    ...
    provider.setCustomParameters({ hd: 'studio27.be' });
